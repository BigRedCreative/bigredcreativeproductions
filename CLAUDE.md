...
### Phase 19D-2 — Cinematic Homepage Hero Media — **next phase, not started**

Starts now that Phase 19D-1 is accepted and committed. Builds the homepage hero's Media Library integration: `None`/`Image`/`Video` modes, a permanent `mediaAssetId` reference (mirroring the exact pattern already proven twice, Portfolio then Services), video poster resolution from the video asset's own `posterMediaAssetId` (no separate hero-specific poster column expected — see the Phase 19C-era architecture report's reasoning), responsive inline presentation first, poster-first loading, `controls`, `playsInline`, `preload="metadata"`, mobile-safe fallback behavior, and no forced autoplay/mute/loop in this initial pass. A background/autoplay cinematic hero treatment remains a later, deliberately separate enhancement, attempted only once the safe inline video path has real usage behind it — not bundled into this phase.
