...
## Full End-to-End Stripe TEST-MODE Acceptance (Phase 21C-2E)

**Status: COMPLETE. TEST MODE ONLY.** This is the piece Phase 21C-2D's own roadmap entry named as the one remaining gap: not a leg proven in isolation (PaymentIntent creation alone, browser submission alone, or webhook processing alone — each already real and verified in 21C-2B/C/D respectively), but the **entire chain proven in one continuous, unbroken session**: a real customer clicking through the real storefront UI, submitting a real Stripe test-mode card payment through the real Payment Element, and that payment being confirmed by a **real, live-delivered signed webhook** — with zero manual intervention, zero resend, zero shortcut. This phase was acceptance/testing only; it touched **zero application source files** — no code, schema, or architecture changed.

### The full chain, genuinely unbroken

```
/store/test-21c2e-e2e-accept (real page load)
  → Add to Cart (real click)
  → /cart (real cart state, $12.34)
  → Continue to Checkout (real click)
  → /checkout details (real form fill: firstName/lastName/email)
  → Review Order (real click) → real POST /api/orders → BRCP-1502 created
  → Submit Order Request (real click) → real POST /api/orders/[id]/payment-intent
  → real Stripe Payment Element mounts (real Stripe.js, real iframes)
  → real card entry: 4242 4242 4242 4242, 12/34, 123, 10001
  → Pay $12.34 (real click) → real Stripe.js confirmPayment()
  → real Stripe test-mode charge succeeds
  → "Payment submitted..." shown, cart clears to CART (0)
  → [no manual action of any kind]
  → real Stripe CLI listener receives the real payment_intent.succeeded event LIVE
  → real POST /api/stripe/webhook, real signature verified, real 200 returned
  → BRCP-1502.paymentStatus: pending → paid
```

Every arrow above is a real action against the real running application — no step was seeded, mocked, or skipped.

### Browser automation approach

No page-render test harness exists in this codebase (confirmed standing fact — no jest/vitest/RTL/Playwright/Puppeteer). Real headless Chrome (`--headless=new --remote-debugging-port`) was driven directly over the Chrome DevTools Protocol via the already-installed `ws` package — the identical approach Phase 21C-2C's own real acceptance used. One genuine technical finding worth recording: Stripe's Payment Element renders its card fields inside a cross-origin iframe that Chrome's screenshot capture visibly **redacts** (a real, deliberate Chrome privacy protection for payment-form content — confirmed by the redacted region's coordinates exactly matching the iframe's own bounding rect, not a bug in this session's tooling). Screenshots were therefore unusable for visually confirming field placement; the reliable approach was attaching a **second, independent CDP session directly to the Stripe iframe's own target** (CDP exposes cross-origin iframes as separately attachable targets) and reading the real DOM field elements (`#payment-numberInput`, `#payment-expiryInput`, `#payment-cvcInput`, `#payment-postalCodeInput`) and their coordinates directly — then dispatching real `Input.dispatchMouseEvent`/`Input.dispatchKeyEvent` sequences scoped to that iframe's own session, confirmed correct by reading each field's own live `.value` back before submitting.

### Synthetic fixture

One product, `prod_test-21c2e-e2e-accept` (slug `test-21c2e-e2e-accept`), fixed-price $12.34, published only for the duration of this run. One resulting real order, customer, and order line — all clearly tagged, all deleted in cleanup (see below).

### Pre-payment state — verified before any card data was submitted

`paymentAccessToken` issued; `stripePaymentIntentId` populated (`pi_3TygJHFjnSocD8E81q6dXmYy`); `stripePaymentIntentAttemptedAt` populated; `paymentStatus: "pending"`; `stripePaymentStatus: "requires_payment_method"`; authoritative amount from `order_lines` (1234) exactly matched `pricingSummary.subtotal` (1234); exactly one `payment.intent_created` audit event; zero `payment.succeeded` events; zero `stripe_webhook_events` row — all confirmed via a direct database read before the card was ever touched.

### Real payment + real webhook — the core result

Card confirmed successfully through the real Payment Element. The browser showed the exact approved wording — *"Payment submitted. We're confirming your payment — you'll receive a follow-up once it's fully processed. No further action is needed right now."* — with the cart correctly cleared to `CART (0)`, and **no** occurrence anywhere of "Paid," "Payment successful," or "Payment complete." The real, live Stripe CLI listener delivered the real `payment_intent.succeeded` event **without any manual resend** — confirmed within the first 5-second poll window, a meaningfully different (and better) outcome than Phase 21C-2D's own troubleshooting history, where the listener/secret sync issue required a one-time manual resend. `BRCP-1502` transitioned `pending → paid`, `paidAt` set, `stripePaymentStatus: "succeeded"`, exactly one `stripe_webhook_events` row (`evt_3TygJHFjnSocD8E81j0rzgGL`, type `payment_intent.succeeded`), exactly one `payment.succeeded` audit event, zero `payment.webhook_anomaly` events. A direct database-wide check confirmed no duplicate order, no duplicate PaymentIntent, no duplicate webhook row — exactly one new order/customer/line/webhook-event beyond the pre-existing real baseline.

### Stripe-side confirmation (read-only)

One dedicated `paymentIntents.retrieve()` call confirmed, directly from Stripe's own system: `livemode: false`, `amount_received: 1234` (exactly matching the authoritative order amount), `currency: "usd"`, `status: "succeeded"`. No further mutation was made to the PaymentIntent.

### Admin verification — an honest, documented gap

The admin order-detail page's own auth boundary was confirmed live (`GET /admin/orders/[id]` unauthenticated → real `307` redirect to `/admin/login`) — but this automated session has no real Google OAuth credentials to complete an actual authenticated sign-in, the same documented constraint every prior admin-action regression test in this codebase's history has hit. Rather than fabricate or bypass authentication, this was verified the same way this project has always verified `requireAdminUser()`-gated logic when a live session isn't available: calling the **exact same `getOrderById()`** the real page renders from (confirmed `paymentStatus: "paid"`), and replicating `setOrderPaymentStatusAction`'s own guard check read-only against the real order (confirmed it would fire — `stripePaymentIntentId !== null` — with the exact approved message). **This is not a substitute for a real authenticated click-through** — stated plainly rather than glossed over. One incidental, non-contradictory finding: since `paymentStatus` is now `"paid"` (a terminal state with zero outgoing transitions in the shared `PAYMENT_STATUS_TRANSITIONS` table), the admin form would already show "no further changes possible" even without the Stripe-linked guard — the guard's real value is for pre-paid Stripe-linked states (`pending`/`failed`), which do have transitions in the shared table it must override.

### Security/leakage scan

Database-side: `paymentAccessTokenHash` confirmed to be a genuine 64-character hex SHA-256 digest, never a raw token; zero secret/card-shaped strings anywhere in the order row or its audit metadata; audit metadata confirmed to exactly match the approved minimal shape (`{provider, orderNumber}` / `{provider, eventType, orderNumber}`). Browser storage: `localStorage["brcp-cart"]` correctly emptied post-payment; `sessionStorage` held only Next.js's own dev-mode debug channel — **zero** checkout-draft key at all, confirming the app's own documented "clear the persisted draft entirely once a real order is confirmed" behavior held in this real run. **Honest gap**: this session did not attach a live console listener to the browser tab, and had no access to the dev server's or the Stripe CLI's own terminal stdout — those three specific checks from the requested scan were not directly performed; database and browser-storage content (the two channels an actual secret could plausibly have leaked into) were both directly and thoroughly checked and found clean.

### Duplicate/idempotency

The real webhook was delivered exactly once, naturally — no manual resend was needed or performed. Per the approved default, this alone is sufficient for the primary E2E proof; duplicate/idempotency protection itself remains covered by Phase 21C-2D's own 30/30 offline suite (which specifically exercises sequential and genuinely-concurrent duplicate delivery). No additional live payment/webhook interaction was added solely to re-prove what's already covered.

### Cleanup

Plan captured and reviewed before executing: delete exactly the synthetic order (`BRCP-1502`, cascades its one `order_lines` row), the synthetic customer, and the synthetic product. **Preserved, exactly as required**: the real Stripe test-mode PaymentIntent (external, untouched, permanent in Stripe's own system), the `stripe_webhook_events` row (`relatedOrderId` auto-nulled via its `ON DELETE SET NULL` FK — the row itself is never deleted), and all three audit rows (`order.created`, `payment.intent_created`, `payment.succeeded` — no FK on `entity_id`, never touched by the order's deletion). `order_number_seq` was never reset or manipulated — `BRCP-1502` is permanently retired, never reused. Post-cleanup, independently re-verified: `orders`/`customers`/`products`/`order_lines` all back to exactly their pre-run real counts (1 each), `stripe_webhook_events` at 2 (both real events, from this phase and Phase 21C-2D, permanently retained), `audit_log` unchanged at 109 rows, `BRCP-1013` completely unaffected throughout, and the synthetic store page confirmed `404` after cleanup.

### Regression re-verification

An 18-check condensed re-run of Phase 21C-2D's own offline mock suite (signature verification, all three event types, every state-precedence rule including the never-downgrade-paid guarantees, amount/currency/livemode anomaly classification, `order_not_found` retryability, sequential and concurrent duplicate delivery, the Stripe-linked admin guard, audit-metadata safety, and `BRCP-1013` non-interference) — **18/18 passed**, confirming zero regression from this phase, which is expected since this phase touched no application source file at all.

### Final quality gates

`npx tsc --noEmit`, `npm run lint`, and `npm run build` all clean at the end of this phase, with the production route manifest confirming no synthetic route of any kind remains.

### What this phase deliberately did not do

No refund implementation. No live-mode configuration or key of any kind (the test-mode-only guards on both the server `sk_test_` check and the client `pk_test_` check held throughout, and the retrieved PaymentIntent independently confirmed `livemode: false`). No payment architecture change — every finding in this phase was either an expected, already-designed behavior or a testing-methodology detail (the CDP iframe-attachment technique), never a code defect requiring a fix. No commit, no push, no merge — this phase's own working tree is clean (identical to `main` plus this CLAUDE.md update), left for review before any commit.
...
- **Phase 21C-2E — Full, single-session end-to-end test-mode payment acceptance — COMPLETE.** See "Full End-to-End Stripe TEST-MODE Acceptance (Phase 21C-2E)" above — the one remaining piece Phase 21C-2D's own roadmap entry named, now proven: a single, continuous real session (real browser storefront click-through, real Stripe Payment Element card submission, real live-delivered signed webhook, zero manual resend) taking `BRCP-1502` all the way from cart to `paymentStatus: "paid"`, independently re-verified against real Neon and Stripe data, with an 18/18 regression re-run confirming zero code-level regression (this phase touched no application source file). Synthetic fixture cleaned up; the real Stripe PaymentIntent, the `stripe_webhook_events` row, and all three audit events permanently retained as evidence.
