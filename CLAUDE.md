...
**Status: complete — a two-layer rate-limiting design (Vercel Firewall for public/IP-level edge protection, configured entirely outside this codebase; a Postgres-backed application-level limiter for authenticated/admin-scoped short-window bursts) is live for Big Red Brain, Creative Studio image generation, and video-upload-token issuance.** This is the first piece of implementation to come out of the Phase 21 security audit below — specifically, the rate-limiting portion of 21H. Everything else in Phase 21 (21A's remaining auth/CSRF work, 21B–21K) remains unstarted; see the Roadmap entry for the current, authoritative state of the rest of Phase 21.
...
### Phase 21 — Security Hardening + Penetration Testing — **in progress (21A-1B/21A-1C complete)**

**A required production/launch security gate — not optional, not satisfied merely because admin authentication already works.** Must cover the entire application, targeting professional small-business application security using OWASP ASVS Level 2-style controls where applicable. A full audit (21A–21K below) was completed first, architecture/report only, before any implementation began. **The rate-limiting portion of 21H is now implemented and real-tested** — see "Rate Limiting (Phase 21A-1B / 21A-1C)" above for the full architecture, exact limits, and real acceptance history. Everything else below — the remainder of 21A (auth/CSRF-specific attack testing), 21B–21G, the rest of 21H (Vercel Firewall's own dashboard configuration for public/IP-level surfaces), and 21I–21K — remains not started. Subsections:
...
- **21H — Rate Limiting + Abuse Protection** — **partially complete**: Big Red Brain, Creative Studio image generation, and video-upload-token issuance are now rate-limited (see "Rate Limiting (Phase 21A-1B / 21A-1C)" above). Still open: contact form, authentication-sensitive endpoints (admin login/OAuth), checkout, order creation — all planned for Vercel Firewall (public/IP-level, dashboard-configured), not yet applied. The existing contact-form same-email cooldown (Phase 18A) is useful but explicitly does **not** substitute for general abuse/rate-limit protection — `POST /api/orders` has been a documented, known gap since Phase 11, and closing it is still open work.
