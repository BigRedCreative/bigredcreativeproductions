...
- **Phase 21C-2E — Full end-to-end test-mode payment acceptance — not started.** Not to be confused with Phase 21C-2B's own narrower real acceptance (PaymentIntent *creation* only) or Phase 21C-2C's own real acceptance (browser payment *submission* only, both documented above) — this sub-phase is the remaining piece: real webhook-driven confirmation (21C-2D) plus a genuine `unpaid → pending → paid` transition for a real test-mode payment, neither of which exists yet.
