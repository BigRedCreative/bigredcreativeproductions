...
// Single-slot hero image editor — manual local path (existing, still
// supported) or "Choose from Media Library" (same toggle-panel/grid
// pattern ProductMediaEditor/LogoPickerField already established).
// Serializes to one hidden JSON field; an empty src means "no hero image"
// (ServiceHero.tsx already falls back to the typographic treatment).
export default function ServiceHeroField({
  name,
  initialImage,
  mediaAssets,
}: {
  name: string;
  initialImage: ServiceImage | undefined;
  mediaAssets: PickerMediaAsset[];
}) {
  const [image, setImage] = useState<ServiceImage>(initialImage ?? { src: "", alt: "" });
  ...
  return (
    <div className="admin-form-row">
      ...
      <p className="admin-form-section-help">
        Shown at the top of the service page. Leave blank to use the site&apos;s typographic fallback treatment.
      </p>
      ...
      <div className="admin-form-row admin-form-row-split">
        ...
        <label>
          ...
          <input
            type="text"
            value={image.src}
            placeholder="/images/services/[slug]/hero.jpg"
